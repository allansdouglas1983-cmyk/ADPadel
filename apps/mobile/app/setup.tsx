...
import { Button, NameInput, Screen, Segmented, Text } from '@/ui';
      ...
      <Screen scroll>
        ...
        <View style={{ gap: spacing.sm }}>
          ...
          {teamA.slice(0, perSide).map((n, i) => (
            <NameInput key={`a${i}`} value={n} onChangeText={(v) => setTeamA((p) => p.map((x, idx) => (idx === i ? v : x)))} />
          ))}
        </View>
        ...
        <View style={{ gap: spacing.sm }}>
          ...
          {teamB.slice(0, perSide).map((n, i) => (
            <NameInput key={`b${i}`} value={n} onChangeText={(v) => setTeamB((p) => p.map((x, idx) => (idx === i ? v : x)))} />
          ))}
        </View>
